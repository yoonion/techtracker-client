...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/auth/discord/status`, {
      method: "GET",
      headers: {
    ...
    const result = (await response.json()) as DiscordStatusResponse;
    if (!response.ok) {
      const message = Array.isArray(result.message)
        ? result.message.join(", ")
        : result.message ?? "Discord 연동 상태 조회에 실패했습니다.";
